...
patch(DateTimePicker.prototype, {
    ...
    getCustomTimeValues(val, index) {
        const defaultTime = this.props.defaultTime || this.props.defaultStartTime || DateTime.local();
        const defaultEndTime = this.props.defaultEndTime || DateTime.local().plus({ hour: 1 });

        const timeSource = index === 1 ? val || defaultEndTime : val || defaultTime;

        return [
            timeSource.hour || DateTime.local().hour + (index === 1 ? 1 : 0),
            timeSource.minute || 0,
            timeSource.second || 0,
        ];
    },
